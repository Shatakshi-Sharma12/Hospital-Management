...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const appointmentsResponse = await axios.get(`https://medcare-7dnb.onrender.com/appoint/user/${storedUser._id}`);
      const upcomingCount = appointmentsResponse.data.appointments.filter(
        apt => new Date(apt.date) > new Date() && apt.status !== 'Cancelled'
      ).length;
      const recordsResponse=await axios.get(`https://medcare-7dnb.onrender.com/health/records`,{
        params:{userId}
      });
  ...
  const fetchHealthProfile = async () => {
    try {
      const response = await axios.get(`https://medcare-7dnb.onrender.com/health/profile/${storedUser._id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const endpoint = `https://medcare-7dnb.onrender.com/health/profile`; // Use a single endpoint for both create and update
