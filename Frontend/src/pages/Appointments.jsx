...
const Appointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axios.get(`https://medcare-7dnb.onrender.com/appoint/user/${userId}`);
  ...
  const handleBookAppointment = async (e) => {
    ...
    try {
      const response = await axios.post("https://medcare-7dnb.onrender.com/appoint/schedule", {
        userId,
  ...
  const handleCancelAppointment = async (appointment) => {
    try {
      await axios.patch(`https://medcare-7dnb.onrender.com/appoint/cancel/${appointment._id}`);
      setAppointments(
  ...
  const handleRescheduleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `https://medcare-7dnb.onrender.com/appoint/reschedule/${selectedAppointment._id}`,
        { newDate: `${date}T${time}` }
      );
  ...
  const handleUpload = async (e, appointmentId) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://medcare-7dnb.onrender.com/appoint/uploadDoc/${appointmentId}`,
        {
